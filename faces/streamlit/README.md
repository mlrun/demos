...
Perform the following steps to deploy an interactive [Streamlit](Streamlit) dashboard for classifying (labelling) and browsing collected images in your Iguazio Data Science Platform ("the platform") environment:

1. On your local computer, edit the [**faces/streamlit/streamlit.yaml**](streamlit.yaml) demo configuration file to replace the `<ACCESS_KEY>` placeholder in the following two locations with a valid access key for your platform environment:
...
4. Run the following command from a command-line shell on your local comupter to deploy the Streamlit dashboard as a service; ensure that current directory has your edited **streamlit.yaml** file or change the path (`-f`) to point to your file:
